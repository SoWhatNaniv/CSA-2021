...
key_checker_data_stuff = open(r"key_checker_data", "rb").read()

s1_encrypted = """41ic3 3@S b391nn1ng T* 93t V3Ry TlrE6 *7 51ttin9 8Y HER 515TER 0n TH3 b@nK, 42D *7 hAVln9 20thing T* D0: *2©3 0R T3lc3 Sh3 H@D p3Ep36 12T* tHE 800k H3r SiST3r wAS r34D1ng, b[T 1t H4D n* 91CT[rE5 0R C02v3R5@t10n5 l2 lT, 'An6 3H4t 1S thE uS3 *F @ 8*0k,' Th0ugHt 41lcE 'wiTH*[t 9iCt[R35 0r ©0nvERS@t1*n?'
...
s2_normal = """Alice was beginning to get very tired of sitting by her sister on the bank, and of having nothing to do: once or twice she had peeped into the book her sister was reading, but it had no pictures or conversations in it, "and what is the use of a book," thought Alice "without pictures or conversation?"
...
data = "{hey_that_is_the_great_puzzle}"

dict_data = {}
for i, c in enumerate(s2_normal):
    if s2_normal[i].lower() in dict_data:
        dict_data[s2_normal[i].lower()].add(s1_encrypted[i])
    else:
        dict_data[s2_normal[i].lower()] = set(s1_encrypted[i])
...
flag_stuff = []
for c in data:
    flag_stuff = flag_stuff + [list(dict_data[c])]

print(flag_stuff)

xxxx = 0 
itera_per = itertools.product(*flag_stuff)
for key in itera_per:
    key_t = "".join(key)
    if xxxx % 1000000 == 0:
        print(key_t)
        
    if check_key(key_t, key_checker_data_stuff):
        print("FOUND " + key_t)
        break
